document.addEventListener('DOMContentLoaded', function () {
    ...
    const pageName = path.split('/').pop().toLowerCase().replace('.html', '') || 'index';
    generateNavigation();
    if (pageName !== 'tools' && !path.toLowerCase().includes('tools.html') && !document.getElementById('toolList')) {
...
function generateNavigation() {
    ...
    let navHTML = `
        <div class="nav-container">
            ...
            <ul class="nav-links">
    `;
    const sortedNavItems = [...navItems].sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
...
function showArticleDetail(articleId) {
    ...
    const contentHtml = typeof marked !== 'undefined' ? marked.parse(article.content) : article.content;
    let detailHTML = `
    ...
    if (typeof hljs !== 'undefined') {
        setTimeout(() => {
            document.querySelectorAll('#articleDetail pre code').forEach((block) => {
